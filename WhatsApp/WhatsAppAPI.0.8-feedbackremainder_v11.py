...
os.chdir(currDir)
#from WhatsAppReadWrite_v6 import GetCurrentEvent
    ...
    try:
        ...
        try:                         
            ...
            #Save max time from df_Chats to table as max time
                ...
                #Process the candidates who as replied back as latest
                df_Responses = df_Latest[df_Latest['Direction']=='inbound'] 
                #df_Responses = df_Latest
                        ...
                        if df_Responses.shape[0]>0:
                            ...
                            for i, row in df_Responses.iterrows():
                                ...
                                if LastQnID != '': #Not able to get last qn as it is a start of conversation or restart of a completed conversation(resubmitting of resume or new interview)                 
                                    ...
                                    #Cases where there is no change in event
        ...
        try:
            ...
            if df_OpenConv.shape[0]>0:  
                ...
                if df_OpenConv.shape[0]>0:
                    ...
                    # df_OpenConv['Event'] = df_OpenConv.apply(lambda x: GetCurrentEvent(x['phone']), axis=1)
        ...
        try:
            ...
            if df_LinkVerified.shape[0]>0:                        
                ...
                for i, row in df_LinkVerified.iterrows():
                    ...
                    ###########################################################################
                    # sQry = "insert into pephire_trans.conversation_history_table values ('"+Phone+"','"+localphone+"','"+str(oid)+"','"+str(uid)+"','"+'Link verified'+"','candidate',now(),'"+'WHATSAPP'+"')"
                    # Flag = db_write(sQry,pephire_db_trans,"others")
                    # if Flag !='1':
        ...
        try:
            ...
            #sQry='select CAST(timeslot_end as CHAR(50)) as timeslotnew_end from pephire_test.shortlisted_candidates_trans where timeslot_end !='"'NULL'"';'
            df_ShortlistedCandidates = pd.DataFrame()
            df_ShortlistedCandidates = db_read(sQry,pephire_db_trans,"select")
           # df_ShortlistedCandidates["timeslot_end"]= pd.to_datetime(df_ShortlistedCandidates["timeslot_end"])
            ...
            if df_Interviewer.shape[0]>0:
                ...
                for i, row in df_Interviewer.iterrows():
                    ...
                    if (datetime.now()>Interviewer_EndTimeNew):
                            print('ll')
                            # df_InterviewerNew =df_Interviewer [df_Interviewer['interview_end_timenew']==interview_end_time]
                            print('111')
                            ...
                            # for j,rw in df_InterviewerNew.iterrows():
                            else:
                                intrvwfeedback='interviewerfeedback'
                                userInt='interviewer'
                                state='incomplete'
                                #insert into candidate stages table interviewer details.
                                print('insert Q')
                                ...
                                print('db status')
                                if Flag !='1':
                                    print('Failed to insert into candidate stages table')
                                    
                                else :
                                    Flag=''
                                    #if the insert is successful, delete it from interview time slots table
                                    print('del Q')
                        ...
                        if df_InterviewerRem.shape[0]==0:
                            ...
                            if remainnderstatusInt!='1':
                                ...
                                print('Interviewer Remainder Question already sent')
                        else:
                            print('interviewerremainder stage already in candidate stages')
        ...
        try:                         
            ...
            #Save max time from df_Chats to table as max time
                ...
                if df_ResponsesInt.shape[0]>0:                 
                    #df_ResponsesInt['Contact'] = df_ResponsesInt.apply(lambda x: FormatPhone(x['ChatId']), axis=1) 
